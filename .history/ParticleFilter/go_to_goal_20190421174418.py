...
def compute_odometry(last_pose, curr_pose, cvt_inch=True):
    '''
...
current_pose = None
ERROR_OFFSET_Y = cozmo.util.Pose(x=0, y=6, z=0, angle_z=cozmo.util.Angle(degrees=0))
...
def CozmoWarehouseWorker:
    ...
    def __init__(self, robot, grid, current_arena_pose, ):
        self.current_arena_pose = current_arena_pose
        self.last_robot_pose = robot.pose
        self.robot = robot
        self.particle_filter = ParticleFilter(grid)
    ...
    async def look_around_until_converge():   
        ...
        while not conf:
            ...
            odometry = compute_odometry(last_pose, curr_pose)
