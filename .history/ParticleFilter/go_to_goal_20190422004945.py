...
class CozmoWarehouseWorker:
    ...
    async def __execute_directions(self, directions):
        self.robot.pose = self.current_arena_pose
    ...
    def update_current_arena_pose(self):
        ...
        coordinate_systems_diff = -self.current_arena_pose.rotation.angle_z.degrees
    ...
    async def localize(self, turn_angle=30):   
        ...
        await self.robot.set_lift_height(0.0).wait_for_completed()
        await self.robot.set_head_angle(degrees(3)).wait_for_completed()
        initial_robot_pose = self.current_robot_pose
        print("INITIAL: ", initial_robot_pose)
        while not conf:
            # move a little 
        ...
        self.pose_offset = degrees(diff_heading_deg(self.robot.pose - initial_robot_pose).rotation.angle_z.degrees, 0)
