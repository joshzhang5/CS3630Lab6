...
class CozmoWarehouseWorker:
    ...
    async def localize(self, turn_angle=20):   
        ...
        while not conf:
            ...
            await self.robot.turn_in_place(angle=degrees(turn_speed)).wait_for_completed()
...
async def run(robot: cozmo.robot.Robot):
    ...
    await cosimo.localize(turn_angle=20)
    await cosimo.drive_to(directions=cosimo.pick_up_pose, drive_speed=50)
    while True:
        await cosimo.pick_up_cube(tries=5)
        await cosimo.drive_to(cosimo.drop_off_directions, drive_speed=50)
        await cosimo.set_down_cube()
        await cosimo.drive_to(cosimo.pick_up_directions, drive_speed=50)
