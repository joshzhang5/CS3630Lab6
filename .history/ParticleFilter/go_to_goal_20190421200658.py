...
class CozmoWarehouseWorker:
    ...
    async def drive_to(directions):
        ...
        if isinstance(directions, (list,)):
            ...
            self.__drive_to_pose(directions)
        print("-" * 20)
    ...
    async def pick_up_cube(self):
        print("-" * 10 + "GETTING CUBE" + "-" * 10)
        cube = await self.robot.world.wait_for_observed_light_cube(timeout=30)
        print("Found cube: %s" % cube)
        picked_up_cube = await self.robot.pickup_object(cube, num_retries=5).wait_for_completed().obj
    ...
    async def __execute_directions(self, directions, drive_speed=50):
        ...
        await self.robot.drive_straight(distance=distance_mm(directions.position.x * grid.scale), speed=speed_mmps(drive_speed)).wait_for_completed()     
        ...
        await self.robot.drive_straight(distance=distance_mm(directions.position.y * grid.scale), speed=speed_mmps(drive_speed)).wait_for_completed()
