...
class CozmoWarehouseWorker:
    ...
    async def __execute_directions(self, directions):
        ...
        await self.robot.drive_straight(distance=distance_mm(directions.position.x * 1.0 * (self.grid.scale + .4)), speed=self.drive_speed).wait_for_completed()     
        ...
        await self.robot.drive_straight(distance=distance_mm(directions.position.y * 1.0 * (self.grid.scale + .4)), speed=self.drive_speed).wait_for_completed()
    ...
    async def localize(self, turn_angle=20):   
        ...
        self.pose_offset = degrees(-(curr_h - self.robot.pose.rotation.angle_z.degrees))
