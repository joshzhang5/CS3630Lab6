...
async def CozmoPlanning(robot: cozmo.robot.Robot):
    ...
    CENTER_NODE = Node((325, 225))
    cmap.set_start(Node((150, 250)))
    # Initialize robot position and start_node
    cosimo = CozmoExplorer(robot, x_0=6.0, y_0=10.0, theta_0=0)
    at_goal = False
    while not at_goal:
        # Look for the goal and go to center and look if not found initially
        if (cosimo.goal_node == None):
            if await cosimo.look_for_goal():
                print("Found goal!")
                continue
            else:
                if await cosimo.go_to_goal(goal_node=CENTER_NODE):
                    await cosimo.look_around_for_goal()
        else:
            at_goal = await cosimo.go_to_goal()        
...
class CozmoExplorer:
    ...
    async def go_to_goal(self, goal_node=None):
        ...
        # print("Goals: ", [str(node.x) + "," + str(node.y) for node in cmap.get_goals()])
        # print("Path: ", [str(node.x) + "," + str(node.y) for node in path])
        for node in path:
    ...
    async def __go_to_node(self, target_node):
        ...
        # detect_obstacles_thread = threading.Thread(target=self.__detect_obstacles)
        # Drive in a straight line to goal node
        # detect_obstacles_thread.start()
        print("Driving ", drive_distance, " mm to towards ", target_node) 
        obstacle_detected = False
        while drive_distance.distance_mm > 0:
            short_drive = cozmo.util.distance_mm(75)
            if (drive_distance.distance_mm < 75):
                "Going to short place!"
                short_drive = drive_distance
            await self.robot.drive_straight(speed=self.DRIVE_SPEED, distance=short_drive).wait_for_completed()
            drive_distance -= short_drive
            obstacle_detected = await self.__detect_obstacles()
            if (obstacle_detected):
                break
        # await drive_task
        # obstacle_detected = await check_task
        # # obstacle_detected = await self.__detect_obstacles()
        # # regardless update robots position, either it has a new start node or it's at the next node
        # print("DONE DRIVING")
        self.__update_pose()
        return not obstacle_detected

        # if obstacle is detected main function needs to know to restart RRT
