...
class CozMap:
    ...
    def compute_smooth_path(self):
        ...
        i = 0
        print("Path is", len(path))
        while i < len(path) - 1:
            farthest_valid_end_node = None
            next_i = i + 1
            for j in range(i, len(path)):
                if not self.is_collision_with_obstacles((path[i], path[j])):
                    farthest_valid_end_node = path[j]
                    next_i = j
            if (farthest_valid_end_node != None):
                new_path.append(farthest_valid_end_node)
            i = next_i
        return new_path
