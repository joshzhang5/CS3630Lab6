...
class CozmoWarehouseWorker:
    ...
    def update_current_arena_pose(self):
        ...
        coordinate_systems_diff = -self.pose_offset.degrees + 10
        print("Initial robot pose was: ", self.current_robot_pose)
        print("Current robot pose is: ", self.robot.pose)
        print("With a diff heading of: ", coordinate_systems_diff)
        diff = rotate_point(self.robot.pose.position.x - self.current_robot_pose.position.x, self.robot.pose.position.y - self.current_robot_pose.position.y, coordinate_systems_diff)
