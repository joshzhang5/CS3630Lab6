...
def CozmoWarehouseWorker:
    ...
    def __init__(self, robot:cozmo.robot.Robot, current_arena_pose):
        ...
        self.drop_off_directions = [Direction(x_coord=3, y_coord=4.5, heading=0), Direction(x_coord=21.75, y_coord=4.5, heading=90), Direction(x_coord=21.75, y_coord=13.75, heading=90)]
        self.pick_up_directions = [Direction(x_coord=21.75, y_coord=4.5, heading=90), Direction(x_coord=3, y_coord=4.5, heading=0), Direction(x_coord=4.5, y_coord=20, heading=90)]
    ...
    async def execute_directions(directions):
        ...
        await self.robot.turn_in_place(angle=Angle(degrees=90)).wait_for_completed()
    ...
    async def localize(self):   
        ...
        conf = False
        self.current_arena_pose = Pose(0,0,0,angle_z=Angle(degrees=0))
        ...
        while not conf:
            ...
            await self.robot.turn_in_place(angle=Angle(degrees=20)).wait_for_completed()
        ...
        self.current_arena_pose = Pose(curr_x , curr_y, 0, angle_z=cozmo.util.Angle(degrees=curr_h))
